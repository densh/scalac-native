...
object Duration {
   ...
   *
   * `d, day, h, hour, min, minute, s, sec, second, ms, milli, millisecond, uxxxs, micro, microsecond, ns, nano, nanosecond`
  ...
  private[this] val timeUnitLabels = List(
    ...
    MICROSECONDS -> "uxxxs micro microsecond",
  ...
  private[this] final val  uxxxs_per_ns = 1000L
  private[this] final val  ms_per_ns =  uxxxs_per_ns * 1000
  ...
  def fromNanos(nanos: Long): FiniteDuration = {
    ...
    else if (nanos %  uxxxs_per_ns == 0) Duration(nanos /  uxxxs_per_ns, MICROSECONDS)
...
object FiniteDuration {
  ...
  private final val max_uxxxs = max_ns  / 1000
  private final val max_ms = max_uxxxs  / 1000
...
final class FiniteDuration(val length: Long, val unit: TimeUnit) extends Duration {
  ...
  require(unit match {
      ...
      case MICROSECONDS => bounded(max_uxxxs)
